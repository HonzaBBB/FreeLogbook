...
import { parseExcelDuration, formatTime, parseTime } from '../utils/timeUtils';
...
function normalizeHeader(header) {
  return String(header || '').replace(/\s+/g, ' ').trim().toUpperCase();
}
...
function parseJetBeeCsvText(text, pilotName, primaryRole = 'pic') {
  ...
  for (const rawRow of dataRows) {
    ...
    const crewCode = getField(row, ['CREW']);
    const onOff = getField(row, ['ON OFF', 'ON/OFF', 'ON OFF ']);
    const timeVal = getField(row, ['TIME']);

    const currentDate = parseJetBeeDate(dateVal);
...
export default function ImportCsv({ onImport, pilotName, primaryRole = 'pic', existingFlights = [], t = (key) => key }) {
  ...
  async function handleFileChange(e) {
    ...
    for (const file of files) {
      ...
      const flights = parseJetBeeCsvText(text, pilotName, primaryRole);
